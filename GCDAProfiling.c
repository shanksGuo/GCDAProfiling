// https://llvm.org/svn/llvm-project/compiler-rt/branches/release_37/lib/profile/GCDAProfiling.c
// fix patch by richliu
...
//--------Add By shanks
struct conflict_file_node {
    char *filename;
    struct conflict_file_node *next;
};
struct conflict_file_list {
    struct conflict_file_node *head, *tail;
};

struct conflict_file_node *zero_file_node = NULL;
struct conflict_file_list current_conflict_list;
static void conflict_file_list_insert(struct conflict_file_list *list, char *filename) {
    struct conflict_file_node *new_node = (struct conflict_file_node *)malloc(sizeof(zero_file_node) + sizeof(char) * (strlen(filename) + 1));
...
static void conflict_file_list_remove(struct conflict_file_list *list) {
    struct conflict_file_node *current = list->head;
    struct conflict_file_node *next = NULL;
    while (current) {
        next = current->next;
...
static struct conflict_file_node* conflict_file_list_find(struct conflict_file_list *list, char *filename) {
    struct conflict_file_node *current = list->head;
    while (current) {
...
static void mark_conflict_file(char *filename) {
    char *filename_copy = malloc(1 + strlen(filename));
    strcpy(filename_copy, filename);
    struct conflict_file_node *exist_node = conflict_file_list_find(&current_conflict_list, filename_copy);
    if (!exist_node) {
        conflict_file_list_insert(&current_conflict_list, filename_copy);
    }
}

static void reflush_if_need(struct conflict_file_list *list) {
    struct conflict_file_node *current = list->head;
    if (current) {
        //删除冲突文件
        while (current) {
            fprintf(stderr, "remove file: -----\n");
            fprintf(stderr, "%s", current->filename);
            remove(current->filename);
            current = current->next;
        }
        //删除所有冲突文件之后，清空链表
        conflict_file_list_remove(list);
        //重新触发覆盖率的写入
...
/*
 * Xcode 7.1.1 has bug dealing with files below, just skip, hot fix by richliu
 */
static int skip_special_file () {
    // melissa todo
    char *skipFilenames[] = {"QuartzCore.gcda", "CoreGraphics.gcda"};
    int n = 2;
    for (int i = 0; i < n; ++i) {
        if (filename!=NULL && strstr(filename, skipFilenames[i])!=NULL) {
            return 1;
        }
    }
    return 0;
...
void llvm_gcda_emit_arcs(uint32_t num_counters, uint64_t *counters) {
    ...
    for (i = 0; i < num_counters; ++i) {
        /*
         * For Xcode 7.1.1, old_ctrs[i] will be very large that lead to counters[i] overflow,
         * Which makes counts equal to zero finally.
         * fix by adding overflow protection by richliu
         */
        if ((uint64_t)10000000 > (uint64_t)(counters[i] + (old_ctrs ? old_ctrs[i] : 0)) ) {
...
void __gcov_flush() {
    ...
    reflush_if_need(&current_conflict_list);
}
